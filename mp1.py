...
def generate_dataset_classification_noise(nb_samples, free_location=False):
    ...
    for i in range(nb_samples):
        ...
        category = np.random.randint(3)
        if category == 0:
            X_n[i], X[i] = generate_a_rectangle_noise(noise, free_location)
        elif category == 1: 
            X_n[i], X[i] = generate_a_disk_noise(noise, free_location)
        else:
            [X_n[i], V_n],[X[i], V] = generate_a_triangle_noise(noise, free_location)
    X = X /255
...
def generate_test_set_classification_noise():
    np.random.seed(42)
    [X_n_test,X_test] = generate_dataset_classification_noise(300, True)
